...
class BasisTranslatorFix(TransformationPass):
    """Translates gates to a target basis by searching for a set of
    translations from a given EquivalenceLibrary.
    ...
    * The composed replacement rules are applied in-place to each op node which
    ...
    def run(self, dag):
        ...
        for key, value in self._qargs_with_non_global_operation.items():
            temp_dict_translate[frozenset(key)] = value

        qarg_local_basis_transforms = {}
        for qarg, local_source_basis in qargs_local_source_basis.items():
            expanded_target = target_basis | temp_dict_translate[qarg]
...
def _basis_search(equiv_lib, source_basis, target_basis, heuristic):
    ...
    while open_set:
        ...
        if {gate_name for gate_name, gate_num_qubits in current_basis}.issubset(
            target_basis
        ):
            ...
            while last_basis != source_basis:
                prev_basis, gate_name, gate_num_qubits, params, equiv = came_from[
                    last_basis
                ]
        ...
        for gate_name, gate_num_qubits in current_basis:
            ...
            for neighbor, params, equiv in neighbors:
                ...
                heappush(
                    open_heap, (est_total_cost[neighbor], next(basis_count), neighbor)
                )
...
def _compose_transforms(basis_transforms, source_basis, source_dag):
    ...
    example_gates = {
        (node.op.name, node.op.num_qubits): node.op for node in source_dag.op_nodes()
    }
    mapped_instrs = {}
...
BasisTranslator = BasisTranslatorFix
