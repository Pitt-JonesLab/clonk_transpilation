...
Sheldon, S., Nation, P. D. & Gambetta, J. M. Validating quantum
computers using randomized model circuits. arXiv:1811.12926 [quant-ph]
(2018).
"""
import base64
import cmath
...
class TwoQubitWeylDecomposition:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        Make explicitly-instantiated subclass __new__  call base __new__
        with fidelity=None
        """
    ...
    def specialize(self):
        ...
        Do update a, b, c, k1l, k1r, k2l, k2r, _is_flipped_from_original
        to round to the specialization. Do not update the global phase,
        since this gets done in generic __init__()
        """
    ...
    def actual_fidelity(self, **kwargs) -> float:
        """Calculates the actual fidelity of the decomposed circuit to the
        input unitary."""
    ...
    def __repr__(self):
        """Represent with enough precision to allow copy-paste debugging of all
        corner cases."""
...
def _closest_partial_swap(a, b, c) -> float:
    """A good approximation to the best value x to get the minimum trace
    distance for Ud(x, x, x) from Ud(a, b, c)"""
    m = (a + b + c) / 3
...
class TwoQubitControlledUDecomposer:
    """Decompose two-qubit unitary in terms of a desired U ~ Ud(α, 0, 0) ~
    Ctrl-U gate that is locally equivalent to an RXXGate."""
    ...
    def _to_rxx_gate(self, angle: float):
        """Takes an angle and returns the circuit equivalent to an RXXGate with
        the RXX equivalent gate as the two-qubit unitary.
...
class TwoQubitBasisDecomposer:
    ...
    def traces(self, target):
        """Give the expected traces :math:`|Tr(U \\cdot Utarget^dag)|` for
        different number of basis gates."""
    ...
    def decomp3_supercontrolled(self, target):
        ...
        ~Ud(pi/4, b, 0), all b, and any target. No guarantees for non-
    ...
    def __call__(
        ...
        """Decompose a two-qubit unitary over fixed basis + SU(2) using the
        best approximation given that each basis application has a finite
        fidelity.

        You can force a particular approximation by passing
        _num_basis_uses.
        """
    ...
    def _get_sx_vz_2cx_efficient_euler(self, decomposition, target_decomposed):
        ...
        This first decomposes each unitary from the KAK decomposition
        into ZXZ on the source qubit of the CNOTs and XZX on the targets
        in order to commute operators to beginning and end of
        decomposition. The beginning and ending single qubit gates are
        then collapsed and re-decomposed with the single qubit
    ...
    def _get_sx_vz_3cx_efficient_euler(self, decomposition, target_decomposed):
        ...
        This first decomposes each unitary from the KAK decomposition
        into ZXZ on the source qubit of the CNOTs and XZX on the targets
        in order commute operators to beginning and end of
        decomposition. Inserting Hadamards reverses the direction of the
        CNOTs and transforms a variable Rx -> variable virtual Rz. The
        beginning and ending single qubit gates are then collapsed and
        re-decomposed with the single qubit decomposer. This last step
        could be avoided if performance is a concern.
        """
    ...
    def num_basis_gates(self, unitary):
        """Computes the number of basis gates needed in a decomposition of
        input unitary."""
