...
class TimeCostAnalysis(AnalysisPass):
    """Return the longest path in a DAGcircuit as a list of DAGOpNodes,
    DAGInNodes, and DAGOutNodes."""
    ...
    def run(self, dag):
        if self.critical_path_only:
            for node in dag.op_nodes():
                if node not in self.property_set["duration_longest_path"]:
...
class EdgeContentionPass(AnalysisPass):
    """Creates a frequency dict for how many times an edge in coupling map is
    used."""
