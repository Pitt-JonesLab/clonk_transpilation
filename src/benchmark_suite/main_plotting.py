    ...
    def fix_labels(labels):
        for index, label in enumerate(labels):
            ...
            if label == "Lattice+AltDiagonals-cx":
                labels[index] = "Lattice+AltDiagonals"+ ("-CX" if not motivation else "")
            ...
            if label == "Modular-RR3-riswap":
                labels[index] = "Tree-I" + (r"-$\sqrt{iSWAP}$" if not motivation else "")
            if label == "Hypercube-riswap":
                labels[index] = "Hypercube" + (r"-$\sqrt{iSWAP}$" if not motivation else "")
            if label == "HypercubeSNAIL-(0, 0)-riswap":
                labels[index] = "Corral$_{1,1}$"+ (r"-$\sqrt{iSWAP}$" if not motivation else "")
            if label == "HypercubeSNAIL-(0, 1)-riswap":
                labels[index] = "Corral$_{1,2}$" + (r"-$\sqrt{iSWAP}$" if not motivation else "")
