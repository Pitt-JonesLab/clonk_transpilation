...
class FakeModular(ConfigurableFakeBackendV2):
    ...
    def __init__(self, twoqubitgate="cx", module_size=4, children=4, total_levels=3):
        ...
        assert children > 1 #I think this would break things with the flattening checks
        ...
        def recursive_foo(qubit_counter, current_level, reserved=[]):
            if current_level == 1:
                ...
                for c_i in range(children):
                    ...
                    qubit_counter, c_module = recursive_foo(qubit_counter, current_level=current_level-1, reserved=reserved[c_i] if len(reserved)>0 else [])
                ...
                temp_list = temp_list[0]
                qubit_counter, c_module = recursive_foo(qubit_counter, current_level=1, reserved=temp_list)
                return qubit_counter, temp_module_qubits

        qubit_counter, _ = recursive_foo(qubit_counter=0, current_level=total_levels, reserved=[])
